...
微信三种接入方式都卡在资质上，个人开发者做不了：

| 方式 | 门槛 |
| --- | --- |
| 微信开放平台「网站应用」 | **企业主体**认证，300 元/年，且要求回调域名**已 ICP 备案** |
| 服务号「网页授权」 | 需认证服务号（企业主体 + 300 元/年） |
| 小程序登录 | 需小程序主体认证 |

本项目跑在境外（免备案）托管上，域名未备案，因此微信授权拿不到回调资格。
个人项目可行的替代是 **GitHub / Gitee / Google** 这类开发者平台 OAuth：
GitHub 已经接好（本节），Gitee 的接入方式与 GitHub 几乎一致，
只要在 allauth 里加一个 provider 即可（需要时再加）。
